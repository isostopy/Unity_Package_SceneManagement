...
[AddComponentMenu("Isostopy/Scene Package/Isos Scene Manager")]
public class IsosSceneManager : MonoBehaviour
{
    private static IsosSceneManager _instance = null;   /// Singleton.
	...
	void OnSceneLoaded(Scene scene, LoadSceneMode mode)
	{
		...
		if (sceneFader != null)
		{
			if (useFadeOnLoad)
			{
				SceneFader.FadeInmidiatly(true);
	...
	IEnumerator AsyncLoadingRoutine(string scene, bool fadeToScene)
	{
		...
		/// Y entiendo que como esta funcion es llamada nada mas cargar la escena de carga, estamos en Awake.			( https://issuetracker.unity3d.com/issues/loadsceneasync-allowsceneactivation-flag-is-ignored-in-awake )
		...
		while (asyncLoad.isDone == false)           /// AsyncOperation.isDone solo es true cuando el progreso ha llegado a 1.
		{
			...
			{
				// Esperar a que haya pasado al menos el tiempo minimo.
	...
	public IsosSceneManager Instance => _instance;
